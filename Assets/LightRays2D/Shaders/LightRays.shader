Shader "Custom/LightRays"{
	Properties{
		...
		_FadeBorders("FadeBorders", Vector) = (0.0,1.0,0.0,1.0)
	}
	SubShader{
        Tags{
        	"Queue"="Transparent"
		...
		Pass{
			...
			fixed4 frag(v2f_img i):SV_Target{
				...
				fixed uMin = _FadeBorders.x;
				fixed uMax = _FadeBorders.y;
				fixed vMin = _FadeBorders.z;
				fixed vMax = _FadeBorders.w;
				
				color.a *= clamp(1.0 - (uMin - i.uv.x) / uMin, 0.0, 1.0) *  clamp(((1 - ((1 - uMax) - (1 - i.uv.x)) / (1 - uMax) ) ), 0.0, 1.0);
				color.a *= clamp(1.0 - (vMin - i.uv.y) / uMin, 0.0, 1.0) *  clamp(((1 - ((1 - vMax) - (1 - i.uv.y)) / (1 - vMax) ) ), 0.0, 1.0);
